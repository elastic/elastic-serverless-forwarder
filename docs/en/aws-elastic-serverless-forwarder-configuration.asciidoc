...
If you want to inject in the expanded events, fields from the root of the JSON object that you are expanding from, you can define `inputs.[].root_fields_to_add_to_expanded_event`. The config takes two different type of value:

. the literal string "**all**", in order to inject all the fields at root level (but for the one you are expanding events from): `root_fields_to_add_to_expanded_event: "all"`
. a list of the names of the fields at root level that you want to inject: for example  `root_fields_to_add_to_expanded_event: ["owner", "logGroup", "logStream"]` 
