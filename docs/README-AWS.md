...
Both config params are optional, and can be set independently from each other: exclude is applied first and then include is applied, so exclude takes precedence if both are supplied.
